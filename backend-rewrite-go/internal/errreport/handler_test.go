...
// #59 — top-level `null` body: Node's strict body-parser 400s before any
// handling; the MaxBytes-wrapped decoder no-ops it, so the null guard must.
// (Oversize still wins over null — *http.MaxBytesError surfaces first.)
func TestErrors_NullBody400(t *testing.T) {
	st := &stubIngest{}
	h := newHandler(st)
	rec := httptest.NewRecorder()
	req := httptest.NewRequest(http.MethodPost, "/errors", strings.NewReader("null"))
	h.Ingest(rec, req)
	if rec.Code != 400 {
