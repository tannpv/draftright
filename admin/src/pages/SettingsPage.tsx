...
function SecretField({ label, value, onChange, placeholder, hint }: {
  ...
  return (
    <div>
      <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>{label}</label>
      ...
      {hint && <p style={{ color: 'var(--muted)', fontSize: 11, marginTop: 4 }}>{hint}</p>}
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ marginBottom: 24 }}>
        <h1 style={{ color: 'var(--text)', fontSize: 22, fontWeight: 700, margin: '0 0 4px' }}>Settings</h1>
        <p style={{ color: 'var(--muted)', fontSize: 13, margin: 0 }}>System configuration, payment providers, and authentication</p>
      </div>
      ...
      {activeTab === 'general' && (
        <>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            <h2 style={{ color: 'var(--text)', fontSize: 16, fontWeight: 600, marginBottom: 16 }}>Environment</h2>
            <div style={{ display: 'flex', gap: 12, marginBottom: 16 }}>
              {(['testing', 'live'] as const).map(env => (
                <button key={env} onClick={() => set('environment')(env)} className="btn" style={{
                  ...
                  border: settings.environment === env ? `2px solid ${env === 'live' ? 'var(--success)' : 'var(--primary)'}` : '2px solid var(--border)',
                  background: settings.environment === env ? (env === 'live' ? 'rgba(19,222,185,0.1)' : 'rgba(93,135,255,0.1)') : 'var(--card)',
                  color: settings.environment === env ? (env === 'live' ? 'var(--success)' : 'var(--primary)') : 'var(--muted)',
            ...
            {isLive && <div style={{ background: 'rgba(255,174,31,0.1)', border: '1px solid rgba(255,174,31,0.2)', borderRadius: 8, padding: '10px 14px', color: 'var(--warning)', fontSize: 13 }}>⚠️ Live mode applies strict rate limits and shorter token expiry.</div>}
          </div>

          {/* Logging & Diagnostics */}
          <div className="card" style={{ marginBottom: 24 }}>
            <h2 style={{ color: 'var(--text)', fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Logging &amp; Diagnostics</h2>
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>Minimum severity that desktop &amp; mobile apps write to their local logs. Applied on each client's next health check (~30s).</p>
            <div style={{ maxWidth: 360 }}>
              <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Client log level</label>
              ...
              <p style={{ color: 'var(--muted)', fontSize: 11, marginTop: 4 }}>“Off” is an absolute kill-switch — clients stop writing logs entirely, including errors.</p>
            </div>
          </div>

          {/* Limits */}
          <div className="card" style={{ marginBottom: 24 }}>
            <h2 style={{ color: 'var(--text)', fontSize: 16, fontWeight: 600, marginBottom: 16 }}>Limits</h2>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div><label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Trial Rewrites/Day</label><input type="number" value={settings.trial_limit} onChange={e => set('trial_limit')(parseInt(e.target.value) || 3)} className="dark-input" min={1} /></div>
              <div><label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Max Input Length</label><input type="number" value={settings.max_input_length} onChange={e => set('max_input_length')(parseInt(e.target.value) || 3000)} className="dark-input" min={100} /></div>
            </div>
          </div>

          {/* Session */}
          <div className="card" style={{ marginBottom: 24 }}>
            <h2 style={{ color: 'var(--text)', fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Session</h2>
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>How long users stay signed in. Access token is short-lived and silently refreshed; session lifetime is the maximum idle window before re-login is required.</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Access Token Lifetime (minutes)</label>
                ...
                <p style={{ color: 'var(--muted)', fontSize: 11, marginTop: 4 }}>Default: 15. Auto-refreshed silently — users don't notice expiry.</p>
              </div>
              <div>
                <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Session Lifetime (days)</label>
                ...
                <p style={{ color: 'var(--muted)', fontSize: 11, marginTop: 4 }}>Default: 90. Forces re-login after this many days of inactivity. Industry standard: 30–90.</p>
              </div>
            </div>
          </div>
          ...
          {/* Languages */}
          <div className="card" style={{ marginBottom: 24 }}>
            <h2 style={{ color: 'var(--text)', fontSize: 16, fontWeight: 600, marginBottom: 8 }}>Translation Languages</h2>
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>Default language is auto-detected from browser/region.</p>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
              {ALL_LANGUAGES.map(lang => {
                ...
                }} style={{ padding: '6px 12px', borderRadius: 8, border: enabled ? '1px solid var(--primary)' : '1px solid var(--border)', background: enabled ? 'rgba(93,135,255,0.15)' : 'var(--card)', color: enabled ? 'var(--primary)' : 'var(--muted)', fontSize: 12, fontWeight: 500, cursor: 'pointer' }}>{lang}</button>;
              })}
            </div>
            <div style={{ marginTop: 12, color: 'var(--muted)', fontSize: 12 }}>{(settings.supported_languages || '').split(',').filter(Boolean).length} of {ALL_LANGUAGES.length} enabled</div>
          </div>
        </>
      )}
      ...
      {activeTab === 'payment' && (
        <>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>QR code payments via Vietnamese banks</p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
              <div>
                <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Bank</label>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 16 }}>
              ...
              <div>
                <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>SePay Mode</label>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>Sends verification, renewal reminder, and payment-failed emails. Domain must be verified in Resend before sending.</p>
            ...
            <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', paddingTop: 12, borderTop: '1px solid var(--border)' }}>
              <div style={{ flex: 1 }}>
                <label style={{ display: 'block', color: 'var(--text)', fontSize: 13, fontWeight: 500, marginBottom: 6 }}>Send test email to</label>
                <input
                  type="email"
                  value={testEmailTo}
      ...
      {activeTab === 'auth' && (
        <>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>For web and mobile Google login. Get credentials from <a href="https://console.cloud.google.com/apis/credentials" target="_blank" rel="noopener" style={{ color: 'var(--primary)' }}>Google Cloud Console</a></p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          ...
          <div className="card" style={{ marginBottom: 24 }}>
            ...
            <p style={{ color: 'var(--muted)', fontSize: 12, marginBottom: 16 }}>For iOS and web Apple login. Get credentials from <a href="https://developer.apple.com/account/resources/identifiers/list" target="_blank" rel="noopener" style={{ color: 'var(--primary)' }}>Apple Developer Portal</a></p>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
